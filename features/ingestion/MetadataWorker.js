/* =====================================================================
   features/ingestion/MetadataWorker.js
   Worker para parsing paralelo de arquivos XML do D365FO
   ===================================================================== */

/**
 * Utilitário para extrair o valor de uma tag XML sem usar um parser completo (performance).
 */
...
/**
 * Extrai múltiplos blocos de uma mesma tag XML.
 */
...
function parseByRegex(xmlText, path) {
  ...
  const fields = extractBlocks('AxTableField', xmlText).map(fb => {
    ...
    return { name: fName, type, extendedDataType: extractTagValue('ExtendedDataType', fb), enumType: extractTagValue('EnumType', fb), sourceModels: [model] };
  }).filter(Boolean);
  ...
  const relations = extractBlocks('AxTableRelation', xmlText).map(rb => {
    ...
    return { name: extractTagValue('Name', rb), relatedTable: relTable, cardinality: extractTagValue('Cardinality', rb), relatedTableCardinality: extractTagValue('RelatedTableCardinality', rb), relationshipType: extractTagValue('RelationshipType', rb), constraints, sourceModels: [model] };
  }).filter(Boolean);
  ...
  const indexes = extractBlocks('AxTableIndex', xmlText).map(ib => {
    ...
    const indexFields = extractBlocks('AxTableIndexField', ib).map(ifb => {
      if (extractTagValue('IncludedColumn', ifb) === 'Yes') return null;
  ...
  return { name: baseName, isExtension: isExt, model, models: [model], tableGroup, primaryIndex, clusteredIndex, fields, relations, indexes };
}
...
async function parsePartition(workerId, files) {
  ...
  for (const entry of files) {
    ...
    try {
      ...
      const fragment = parseByRegex(await file.text(), entry.path);
      if (fragment) mergeFragment(tableMap, fragment);
    } catch (e) {
      errors += 1;
    }
    if (totalProcessedInBatch >= BATCH_SIZE) {
      self.postMessage({ type: 'batch_result', workerId, processed: totalProcessedInBatch, errors, tables: finalizeAggTables(tableMap), durationMs: performance.now() - tStartWorker });
      tableMap = new Map();
  ...
  self.postMessage({ type: 'result', workerId, processed, errors, tables: finalizeAggTables(tableMap), extensions: [], metrics: { totalMs: tEndWorker - tStartWorker, avgMsPerFile: ((tEndWorker - tStartWorker) / processed).toFixed(3) } });
}

self.onmessage = (evt) => {
  const msg = evt.data || {};
  if (msg.type === 'parsePartition') {
    parsePartition(Number(msg.workerId || 0), Array.isArray(msg.files) ? msg.files : []).catch(err => {
      self.postMessage({ type: 'error', workerId: Number(msg.workerId || 0), message: err?.message || 'Falha no metadata-worker.' });
    });
  }
};